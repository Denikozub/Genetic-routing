...
- void init() : randomly initialize a gene with values in range
...
- bool survived() const : according to chance check if gene escaped killing attempt
  formula survive_chance = chance / (chance + 1) makes it easier to survive for genes with high chance
...
- Gene mutate() : perform either Wang greedy hybrid operator (>= 6 points) or one-point mutation:
